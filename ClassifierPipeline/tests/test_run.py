...
def _import_run_module(monkeypatch, dummy_logger):
    ...
    fake_tasks.prepare_pre_ingest_run = lambda filename, proj_home_path=None: f"/prepared/{filename or 'pre-ingest'}_classified.tsv"
...
def test_batch_pre_ingest_records_prepares_output_once_and_reuses_output_path(monkeypatch, dummy_logger, tmp_path):
    ...
    captured = []
    prepared = []
    module.prepare_pre_ingest_run = lambda filename, proj_home_path=None: (prepared.append((filename, proj_home_path)) or f"/prepared/{filename}_classified.tsv")
    module.task_update_record.delay = lambda message: captured.append(message)

    module.batch_pre_ingest_records(str(records), batch_size=1)

    assert prepared == [("pre.tsv", module.proj_home)]
    assert [batch[0]["output_path"] for batch in captured] == ["/prepared/pre.tsv_classified.tsv", "/prepared/pre.tsv_classified.tsv"]
